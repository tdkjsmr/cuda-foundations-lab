...
V0 建立连续读写的实际带宽参考。V1 把连续输出写入改为跨步写入。V2 通过 Shared Memory Tiling 恢复连续 Global Load 和 Store，但保留未 Padding Tile 的 Bank Conflict；Padding 将在 `v1.3` 作为唯一主要变化加入。

## 目录关系

```text
include/                    CUDA 错误检查、Event 计时、统计和公共接口
src/transpose/transpose.cu  V0/V1/V2 Kernel、Host 启动逻辑和演示 main
tests/transpose_test.cu     CPU Reference、正确性、边界和特殊位模式测试
benchmarks/                 独立 Copy/Naive/Tiled 稳定态性能测量
